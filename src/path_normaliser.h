...
std::string Code(const std::string& segment) {
    std::stringstream out;
    ...
    while (i < segment.length()) {
        if (segment[i] == '%' &&
            ...
            // PCT Encoded.
            uint16_t pct_encoded = ConvertCharHexToDecim(segment[i + 1]) * 16 + ConvertCharHexToDecim(segment[i + 2]);
...
public:
    static std::vector<std::string> getHierarchicalSegments(const std::string& path) {
    ...
    static std::vector<std::string> removeDotSegments(const std::vector<std::string>& segments) {
    ...
    static std::vector<std::string> codeIfNecessary(const std::vector<std::string>& segments) {
        std::vector<std::string> coded_segments;

        for (const auto& segment: segments) {
            coded_segments.emplace_back(Code(segment));
        }

        return coded_segments;
    }

    explicit PathNormaliser(const std::string& path) noexcept:
        _path_segments(PathNormaliser::getHierarchicalSegments(path)) {
        // Empty on purpose.
    }

    std::string normalise() const {
        std::vector<std::string> removed_dots = PathNormaliser::removeDotSegments(_path_segments);
        std::vector<std::string> coded_segments = PathNormaliser::codeIfNecessary(removed_dots);

        std::stringstream out;
        for (size_t i = 0; i < coded_segments.size(); i++) {
            out << coded_segments[i];

            if (i < coded_segments.size() - 1) {
                out << kPathSeparator;
            }
        }
    ...
    PathNormaliser(const PathNormaliser& that) = default;
    PathNormaliser& operator=(const PathNormaliser& that) = default;
    PathNormaliser(PathNormaliser&& that) = default;
    PathNormaliser& operator=(PathNormaliser&& that) = default;

    ~PathNormaliser() = default;
